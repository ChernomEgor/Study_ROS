...
class Controller_turtle(Node):
    def __init__(self):
        ...
        self.goal_turtle_subscription = self.create_subscription(
            Point, "turtle_goal", self.get_turtle_goal, 10
        )
        self.turtle_pose_subscription = self.create_subscription(
            Pose, "/turtle1/pose", self.get_turtle_pose, 10
        )
    ...
    def get_turtle_pose(self, msg):  # Функция принимающая текущие координаты черепахи
        ...
        self.get_logger().info(
            f"Current pose x:{self.xt:.2f}, y:{self.yt:.2f}, theta:{self.theta:.2f}"
        )
    ...
    def control_loop(self):  # Функция контролирующая все процессы
        if self.xg == None or self.yg == None:
            return

        if self.xt is None or self.yt is None or self.theta is None:
            self.get_logger().warn(
                "Waiting for turtle pose data...", throttle_duration_sec=2.0
            )
            return
        p = math.sqrt((self.xg - self.xt) ** 2 + (self.yg - self.yt) ** 2)

        if p < 0.1:  # Если черепаха у цели
            twist = Twist()
            twist.linear.x = 0.0
            twist.angular.z = 0.0
...
def main(args=None):
    ...
    rclpy.spin(node)  # Держит узел запущеным
    node.destroy_node()
    rclpy.shutdown()
